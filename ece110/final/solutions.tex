...
\begin{enumerate}
    ...
    Solution:
    \( P_{avg} = \frac{P_1t_1 + P_2t_2}{T} = \frac{(15)(12) + (5)(8)}{20} = \frac{180 + 40}{20} = \frac{220}{20} = 11 \textnormal{W} \)
    \item What is the RMS Voltage of this waveform? \\
    Solution:
    \( P_{avg} = \frac{V_{rms}^2}{R} \Rightarrow V_{RMS} = \sqrt{P_{avg}R}= \sqrt{(11)(10)} = 10.49 \textnormal{V}\)
\end{enumerate}
...
\end{center}
Solution: \( DC = \frac{T_{on}}{T} = \frac{2}{4} = \frac{1}{2} \)
...
Solutions: 
    First, solve for the open circuit voltage by leaving an open circuit between the two terminals. Define \(I_1\) as the current running to the right through the 5ohm resistor. Define \(I_2\) as the current running down through the 20ohm resistor. 
    \\A KVL gives:
    \( 15 - 5I_1 - 20I_2 = 0\)
    \\ A KCL gives:
    \( I_1 + 3 = I_2 \)
    \\ Then, solve the system of equations to get \(I_2 = \frac{8}{5} A\). Finally, the voltage across the 20ohm resistor is \(\frac{8}{5}*20 = 32 V\) by Ohm's Law. Since there is no current running through the 4ohm resistor, there is no voltage drop across it. So, \(V_{oc} = 32 V\).
    \\ Then, solve for effective resistance by shorting all the voltage sources and opening all the current sources. Then,
    \\ \(R_{eff} = \frac{1}{\frac{1}{5}+\frac{1}{20}} + 4 = 4+4 = 8 \) ohms.
    \\ Then, we know that \(I_{sc} = \frac{V_{oc}}{R_{}eff} = \frac{32}{8} = 4 A\)
...
KVL Loops are shown above with the assumption that the Voltage source has extremes high or low enough that they will conduct current.
\begin{align*}
    V_{out} = 2V-1V = 1V.
\end{align*}
